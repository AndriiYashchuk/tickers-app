...
type ApiItem<T> = (req?: NextRequest, adapter?: AxiosAdapter) => Promise<AxiosResponse<T>>
...
const getApi = (instance: AxiosInstance): Api => ({
  getCurrentUser: (request?: NextRequest, adapter?: AxiosAdapter): Promise<AxiosResponse<{ currentUser: User}>> => {
    let cookie;
    if(!request){
